...
class Http extends File
{
    ...
    {
        $headers = array_change_key_case(get_headers(
            (!str_starts_with($path, $this->getScheme()) ? $this->getScheme() : '') . $path,
            1), CASE_LOWER);
        $status = $headers[0] ?? '';
    ...
    public function getAbsolutePath($basePath, $path, $scheme = null)
    {
        ...
        if (0 === strpos((string)$path, (string)$basePath)) {
            return !str_starts_with($path, $this->getScheme()) ?
                $this->getScheme() . $path :
                $path;
        }

        return !str_starts_with($path, $this->getScheme()) ?
            (!str_starts_with($basePath, $this->getScheme()) ? $this->getScheme() : '') . $basePath . $path :
            $basePath . $path;
    }
